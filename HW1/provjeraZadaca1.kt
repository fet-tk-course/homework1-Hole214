...
class internEngineer(
    ...
    prezime : String,
    ekspertize : List<String>,
    var trajanjePrakse : Int
) : Inzenjer(ime, prezime, 0, "Intern", ekspertize){
    ...
    override fun ispisInfo(){
        println("Inzenjer: $ime $prezime, godine iskustva: $godIskustva, titula: $titula, trajanje prakse: $trajanjePrakse mjeseci")
    }
}

fun internEngineerStats(pripravnici : List<internEngineer>) {
    var ukupnoPrakse : Int = 0
    for(x in pripravnici){
        ukupnoPrakse += x.trajanjePrakse
    }

    var prosjecnoPrakse : Int = ukupnoPrakse / pripravnici.size

    println("Ukupno trajanje: $ukupnoPrakse mjeseci, prosjek $prosjecnoPrakse mjeseci")
...
fun main() {
    val listaInzenjera : List<Inzenjer> = listOf(
        ...
        internEngineer("Adnan", "Catic", listOf("Kotlin"), 6),
        internEngineer("Lejla", "Kadic", listOf("Kotlin"), 12),
        internEngineer("Amar", "Kesetovic", listOf("Kotlin"), 3)
    ...
    val interni = listaInzenjera.filterIsInstance<internEngineer>()
    internEngineerStats(interni)
